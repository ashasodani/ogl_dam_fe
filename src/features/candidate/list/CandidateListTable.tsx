...
const UserListTable = ({ tableData }: { tableData?: UsersType[] }) => {
  ...
  const [candidateManageList, setCandidate] = useState<any[]>([])
  const dispatch = useDispatch<AppDispatch>()
  const candidateManage = useSelector(getCandidateManageList)
  ...
  const fetchData = async (page = 1, perPage = 10) => {
    ...
    await dispatch(getCandidateManage(params))

    // setLoading(false);
  }

  useEffect(() => {
    setFilteredData(candidateManage.data)
  }, [candidateManage])
  console.log('filteredData', filteredData)
  console.log('candidateManage', candidateManage)
  ...
  const columns = useMemo<ColumnDef<UsersTypeWithAction, any>[]>(
    () => [
      ...
      columnHelper.accessor('role', {
        header: 'Role',
        cell: ({ row }) => <Typography>{row.original.role.name}</Typography>
      }),
      columnHelper.accessor('level', {
        header: 'Level',
        cell: ({ row }) => <Typography>{row.original.role.name === 'Admin' ? '--' : row.original.level.charAt(0).toUpperCase() + row.original.level.slice(1)}</Typography>
      }),
      // columnHelper.accessor('role', {
      //   header: 'Role',
      //   cell: ({ row }) => (
  ...
  const table = useReactTable({
    ...
    pageCount: candidateManage?.meta?.last_page || -1,
    filterFns: {
      fuzzy: fuzzyFilter
    },
    state: {
      ...
      globalFilter,
      pagination: {
        pageIndex: candidateManage?.meta?.current_page - 1 || 0,
        pageSize: candidateManage?.meta?.per_page || 10
      }
    },
 ...
 */
  ...
  const handleConfirmDelete = async () => {
    if (selectedId) {
      try {
        const res = await dispatch(deleteCandidateManage(selectedId) as any)
  ...
  const handleCreateNew = () => {
    ...
    router.push('/apps/candidate/create')
  }
    ...
    <>
      ...
      <Card>
        ...
        <TablePagination
          ...
          component='div'
          className='border-bs'
          count={candidateManage?.meta?.total || 0}
          rowsPerPage={candidateManage?.meta?.per_page || 10}
          page={(candidateManage?.meta?.current_page || 1) - 1}
          onPageChange={(_, page) => {
