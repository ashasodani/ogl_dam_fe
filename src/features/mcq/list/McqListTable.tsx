...
import {
  getMcqAssessManage,
  getMcqAssessManageList,
  getMcqAssessManageLoader,
  deleteMcqManage
} from '@store/slices/mcqAssesSlice'
...
const McqListTable = ({ tableData }: { tableData?: UsersType[] }) => {
  // States
  ...
  const testAssess = useSelector(getMcqAssessManageList)
  const testAssessLoader = useSelector(getMcqAssessManageLoader)
  const router = useRouter()

  const handleEdit = (id: string) => {
    router.push('/apps/mcqtest/view/' + id)
  }
  ...
  const fetchData = async (page = 1, perPage = 10) => {
    ...
    await dispatch(getMcqAssessManage(params))

    // setLoading(false);
  }

  useEffect(() => {
    setFilteredData(testAssess.questions || [])
  }, [testAssess])
      ...
      columnHelper.accessor('test_name', {
        ...
        cell: ({ row }) => (
          <div className='flex items-center gap-3'>
            <div className='flex flex-col'>
              <Typography color='text.primary' className='font-medium'>
                {row.original.name?.length > 60 ? `${row.original.name.substring(0, 60)}...` : row.original.name}
              </Typography>
              <Typography variant='body2'>
                {row.original.title?.length > 60 ? `${row.original.title.substring(0, 60)}...` : row.original.title}
              </Typography>
            </div>
          </div>
        )
        ...
        cell: ({ row }) => ( <div className='flex items-center gap-2'>
            <div className='flex flex-col'>
              ...
              <Typography variant='body2'>
                ...
                {row.original.difficulty.charAt(0).toUpperCase() + row.original.difficulty.slice(1)}
              </Typography>
            </div>
          </div>)
      }),
        ...
        cell: ({ row }) => (
          <div className='flex items-center gap-0.5'>
            ...
            <IconButton size='small' onClick={() => handleEdit(row.original.id)}>
              <Link href={getLocalizedUrl('/apps/mcqtest/view', locale as Locale)} className='flex'>
  ...
  const handleConfirmDelete = async () => {
    if (selectedId) {
      ...
      try {
        ...
        const res = await dispatch(deleteMcqManage(selectedId) as any);
  ...
  const handleCreateNew = () => {
    ...
    router.push('/apps/mcqtest/create')
  }
