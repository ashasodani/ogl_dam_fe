    ...
    dispatch(login(data))
      .then((response: any) => {
        ...
        if (response?.payload?.status) {
          ...
          if(response.payload.data.user.role.name==="Candidate"){
            const redirectURL = searchParams.get('redirectTo') ?? '/candidate-dashboard';

             router.replace(getLocalizedUrl(redirectURL, locale as Locale))
          }else{
            const redirectURL = searchParams.get('redirectTo') ?? '/apps/import/list'
          ...
          // const redirectURL = searchParams.get('redirectTo') ?? '/apps/candidate/list'

          // router.replace(getLocalizedUrl(redirectURL, locale as Locale))

          //toast.success(response?.payload?.message)
          //router.push('/candidate')
        } //else if (response?.error)toast.error(response?.error?.message || "Network response was not ok");
      })
      .catch((error: any) => {
        try {
          setIsLoading(false)
          sad.a, (error = JSON.parse(error))
        } catch {
          setIsLoading(false)
          error = { message: [error] }
        }
